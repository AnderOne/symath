...
t_tree::h_node t_tree::t_node_var::cpy() const {
	return own.gener(std::string(1, ind))->mul(num);
}

t_tree::h_node t_tree::t_node_num::cpy(const t_tree &_own) const {
	return _own.gener(num);
}
t_tree::h_node t_tree::t_node_num::cpy() const {
	return own.gener(num);
}

#define __DEF_ITEM_BIN_CPY(ind, op) \
t_tree::h_node \
t_tree::t_node_##ind::cpy(const t_tree &_own) const {\
	return _own.gener(#op, arg[0], arg[1])->mul(num);\
}\
t_tree::h_node \
t_tree::t_node_##ind::cpy() const {\
